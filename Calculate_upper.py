...
def calculate_upper(filename, obs, config_data):
    ...
    if verbose == 0.0:
        print('Processing %s: %d x %d' %
              (os.path.split(filename)[1], image.shape[1], image.shape[0]))
    ...
    obj = photometry.get_objects_sextractor(
        image, mask=mask, r0=1, aper=5.0, wcs=wcs, gain=gain)
    ...
    print("obs.catalogcalib", obs.catalogcalib,
          obs.filtercalib, obs.filtercalib)
    cat = catalogs.get_cat_vizier(ra0, dec0, sr0, obs.catalogcalib, filters={
                                  f'obs.filtercalib': '<22'})
    ...
    wcs = pipeline.refine_astrometry(
        obj, cat, 5*pixscale, wcs=wcs, order=0, cat_col_mag=cat_col_mag, verbose=True)
    ...
    m = pipeline.calibrate_photometry(obj, cat, pixscale=pixscale, cat_col_mag=cat_col_mag,
                                      cat_col_mag1=cat_color_mag1, cat_col_mag2=cat_color_mag2, order=0, verbose=True)
    ...
    psf_model, psf_snapshots = psf.run_psfex(
        image, mask=mask, gain=gain, checkimages=['SNAPSHOTS'], order=0, verbose=True)
    ...
    for _ in tqdm(range(100)):
        ...
        sim = pipeline.place_random_stars(
            image1, psf_model, nstars=20, minflux=0.05, maxflux=1e6, wcs=wcs, gain=gain, saturation=50000)
        # stop
        ...
        obj1 = photometry.get_objects_sextractor(
            image1, mask=mask | mask1, r0=1, aper=5.0, wcs=wcs, gain=gain, minarea=3, sn=0.05)
        obj1['mag_calib'] = obj1['mag'] + zero_fn(obj1['x'], obj1['y'])

        # Positional match within FWHM/2 radius
        oidx, sidx, dist = spherical_match(
            obj1['ra'], obj1['dec'], sim['ra'], sim['dec'], pixscale*np.median(obj1['fwhm'])/2)
        # Mark matched stars
    ...
    plt.errorbar(sims['mag_calib'][idx], (sims['mag_measured'] - sims['mag_calib'])
                 [idx], sims['magerr_measured'][idx], fmt='.', capsize=0, alpha=0.5)
    ...
    h0, b0, _ = plt.hist(sims['mag_calib'], range=[
                         12, 22], bins=50, alpha=0.3, label='All simulated stars')
    h1, b1, _ = plt.hist(sims['mag_calib'][sims['detected']], range=[
                         12, 22], bins=50, alpha=0.3, label='Detected')
    h2, b2, _ = plt.hist(sims['mag_calib'][idx], range=[
                         12, 22], bins=50, alpha=0.3, label='Detected and unflagged')
    ...
    for mag in mags_bins:
        indice_cutmag = np.where(
            (mag_calib >= mag) & (mag_calib < (mag+step)))[0]
        if len(indice_cutmag) > 1:
            ...
            cut_percentile = np.where(
                (SNR_cutmag < percentile50) & (SNR_cutmag > percentile25))[0]
            indice_filter = np.concatenate(
                (indice_filter, indice_cutmag[cut_percentile]), axis=0)
    ...
    z, res, _, _, _ = np.polyfit(
        mag_calib_cut, np.log10(SNR_cut), 2,  full=True)
    # print("residus",res)
    p = np.poly1d(z)
    propre_x = np.arange(13, 25, 0.1)
    indice_value = np.where((pow(10, p(propre_x)) > 5.0))

    PLOT_UPPER = [mag_calib, SNR, 'Simulated stars', mag_calib_cut, SNR_cut, obs.filters, 'Cut', propre_x, pow(10, p(
        propre_x)), "Upper lim. "+str(np.round(propre_x[indice_value][-1], 1)), 5, 'Detection limit S/N=5', 'Measured magnitude', 'Measured signal to noise ratio']
