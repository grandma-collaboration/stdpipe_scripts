...
def check_withinregion(target_ra, target_dec, target_radius, src_ra, src_rec):
    ...
    dist = astrometry.spherical_distance(src_ra, src_rec, target_ra, target_dec)
...
def detection_source(filename, repo_photo, detection, obs, verbose, mask_simage_threshold, show_preprocimg, show_masksimg, cat_ref, stars_threshold, filter_ref, cat_col_mag_color, cat_col_mag1_color, cat_col_mag2_color, show_calib, show_candidate, template_type, template_ref, show_photorefimage, cat_colsous_mag, show_diffimg, sextractor_default, target_ra, target_dec, target_radius,config_data):
    ...
    obj = photometry.measure_objects(obj, image, mask=mask, fwhm=fwhm, gain=gain, aper=1.0, bkgann=[5, 7], sn=3, verbose=True)
    if verbose is True:
    ...
    cat = catalogs.get_cat_vizier(center_ra, center_dec, center_sr, cat_ref, filters={filter_ref:'<'+"19"})
    if verbose is True:
        print(len(cat), 'catalogue stars')

    # Let's use SCAMP for astrometric refinement.
    wcs = pipeline.refine_astrometry(obj, cat, 5*pixscale, wcs=wcs, method='scamp', cat_col_mag=filter_ref, verbose=True)
    ...
    print("cat_col_mag_color",cat_col_mag_color,"cat_col_mag1_color",cat_col_mag1_color,"cat_col_mag2_color",cat_col_mag2_color)
    #ts.sleep(250)
    m = pipeline.calibrate_photometry(obj, cat, sr=3/3600, cat_col_mag=cat_col_mag_color, cat_col_mag1=cat_col_mag1_color, cat_col_mag2=cat_col_mag2_color, max_intrinsic_rms=0.02, order=2, verbose=True)
    if verbose is True:
        print( m['zero_fn'])
    #ts.sleep(120)
    ...
    if detection=="cat":
        ...
        candidates = pipeline.filter_transient_candidates(obj, cat=cat, sr=5/3600, vizier=['vsx'], verbose=True)
        plt.clf()
        for i,cand in enumerate(candidates):
            if check_withinregion(target_ra,target_dec,target_radius,cand['ra'],cand['dec']) == 1.0:
            	print('Candidate %d with mag = %.2f +/- %.2f at x/y = %.1f %.1d and RA/Dec = %.6f %.6f' % (i, cand['mag_calib'], cand['mag_calib_err'], cand['x'], cand['y'], cand['ra'], cand['dec']))
            	cutout = cutouts.get_cutout(image, cand, 20, mask=mask, header=header)
            	cutout['template'] = templates.get_hips_image(template_ref, header=cutout['header'])[0]
            	candidateswithin.append([cand['mag_calib'], cand['mag_calib_err'], cand['ra'], cand['dec'],cand['mag_calib'], cand['mag_calib_err']])
            	plots.plot_cutout(cutout, qq=[0.5, 99.9], stretch='linear')
            	plt.savefig(os.path.join(repo_photo, "Candidate-"+str(np.round(cand['ra'],4))+"_"+str(np.round(cand['dec'],4))+"WITHINTARGET.png"))
            	plt.clf()
            else:
            	cutout = cutouts.get_cutout(image, cand, 20, mask=mask, header=header)
            	cutout['template'] = templates.get_hips_image('flux-Rp/I/350/gaiaedr3', header=cutout['header'])[0]
            	candidateswithin.append([cand['mag_calib'], cand['mag_calib_err'], cand['ra'], cand['dec'],cand['mag_calib'], cand['mag_calib_err']])
            	plots.plot_cutout(cutout, qq=[0.5, 99.9], stretch='linear')
            	plt.savefig(os.path.join(repo_photo, "Candidate-"+str(np.round(cand['ra'],4))+"_"+str(np.round(cand['dec'],4))+"NOTINTARGET.png"))
            #plt.show()
    ...
    if detection=="soustraction":
							...
							if template_type=="cat":
											...
											print('flux-Rp/I/350/gaiaedr3',template_ref,"image_shape1",image.shape[1]*2,"image_shape2",image.shape[0]*2)
											img_ref_tmp=repo_photo+"/tmp.fits"
											print(img_ref_tmp)
											tmpl = templates.get_hips_image(template_ref, wcs=wcs[::0.5,::0.5], width=image.shape[1]*2, height=image.shape[0]*2, get_header=False,local_file_path=img_ref_tmp,verbose=True)
											#print(tmpl)
											#np.save(tmpl)
							...
							tobj = photometry.get_objects_sextractor(tmpl, mask=~np.isfinite(tmpl), wcs=wcs, aper=3, gain=1.0)
							print(cat,cat_col_mag_color)
							m_tmpl = pipeline.calibrate_photometry(tobj, cat, sr=1/3600, cat_col_mag=cat_col_mag_color,max_intrinsic_rms=0.1, verbose=True) #cat_col_mag_err='e_RPmag'
							...
							if config_data["show_photorefimage"]=="True":
											...
											plt.ylim(-2, 2)
											plt.savefig(os.path.join(repo_photo, "Photometry_referenceimage.png"))
											plt.clf()


							tmask = templates.mask_template(tmpl, cat, cat_col_mag=cat_col_mag_color, cat_saturation_mag=15, wcs=wcs, dilate=3, verbose=True)
							...
							res = subtraction.run_hotpants(image-bg, tmpl-tbg, mask=mask, template_mask=tmask, get_convolved=True, get_scaled=True, get_noise=True, image_fwhm=fwhm, template_fwhm=1.5, image_gain=gain, template_gain=1e6, err=True, verbose=True)
							...
							sobj,segm = photometry.get_objects_sextractor(diff, mask=mask|tmask|dmask, err=ediff, edge=10, wcs=wcs, aper=2.0, extra_params=['CLASS_STAR', 'NUMBER'], extra={'SEEING_FWHM':fwhm, 'STARNNW_NAME':sextractor_default}, checkimages=['SEGMENTATION'], psf='/tmp/psf.psf', verbose=True)
							...
							for i,cand in enumerate(candidates):
											...
											if show_candidate=="True":
															...
															if check_withinregion(target_ra,target_dec,target_radius,cand['ra'],cand['dec']) == 1.0:
																			...
																			print([cand['mag_calib'], cand['mag_calib_err'], cand['ra'], target_obj['dec'],cand['mag_calib'], cand['mag_calib_err']])
																			candidateswithin.append([cand['mag_calib'], cand['mag_calib_err'], cand['ra'], target_obj['dec'],cand['mag_calib'], cand['mag_calib_err']])
																			plt.savefig(os.path.join(repo_photo, "Candidate-"+str(np.round(cand['ra'],5))+"_"+str(np.round(cand['dec'],5))+"WITHINTARGET.png"))
																			plt.clf()
															else:
																			cutout = cutouts.get_cutout(image, cand, 20, mask=mask, header=header)
																			cutout['template'] = templates.get_hips_image(template_ref, header=cutout['header'])[0]
																			candidateswithin.append([cand['mag_calib'], cand['mag_calib_err'], cand['ra'], cand['dec'],cand['mag_calib'], cand['mag_calib_err']])
																			plots.plot_cutout(cutout, qq=[0.5, 99.9], stretch='linear')
																			plt.savefig(os.path.join(repo_photo,"Candidate-"+str(np.round(cand['ra'],5))+"_"+str(np.round(cand['dec'],5))+"NOTINTARGET.png"))
																			plt.clf()
...
def compare_candidates(repo_photo,fits_file,xrt_ra,xrt_dec,xrt_radius):
    detection="cat"
    candidates_sous = np.load(os.path.join(repo_photo, "Cross-Match", "Candidates_"+str(detection)+".npy"))

    detection="cat"
    candidates_cat = np.load(os.path.join(repo_photo, "Cross-Match", "Candidates_"+str(detection)+".npy"))

    candidates_cross=[]
    for i,candcat in enumerate(candidates_cat):
        for j,candsous in enumerate(candidates_sous):
            val=check_withinregion(candsous['ra'],candsous['dec'],1/3600.,candcat['ra'],candcat['dec'])
            if val==1:
                val_xrt = check_withinregion(xrt_ra,xrt_dec,xrt_radius,candcat['ra'],candcat['dec'])
                if val_xrt == 1:
                    print('Candidate with CATALOGUE mag = %.2f +/- %.2f , RA/Dec = %.4f %.4f' % (candcat['mag_calib'], candcat['mag_calib_err'], candcat['ra'], candcat['dec']))
                    print("and")
                    print('Candidate with SOUS mag = %.2f +/- %.2f , RA/Dec = %.4f %.4f' % (candsous['mag_calib'], candsous['mag_calib_err'], candsous['ra'], candsous['dec']))
                    candidates_cross.append([candcat['mag_calib'], candcat['mag_calib_err'], candcat['ra'], candcat['dec'],candsous['mag_calib'], candsous['mag_calib_err']])
    return candidates_cross


def create_logs(repo_photo, fits_file, obs, candidates_cross, upperlimit, config):
    name = (fits_file.split("/")[-1]).split("_")[0]+"_"+obs.instru+"_"+obs.filters+"_"+str(np.round(obs.mjd,3))+"_photometry.csv"
    ...
    with open(repo_photo+"/"+name, 'a+') as output:
        ...
        if len(candidates_cross) > 0:
             for i in np.arange(len(candidates_cross)):
                ...
                sentence=str(obs.obserdate)+","+str(obs.mjd)+","+str(obs.instru)+","+str(obs.filters)+","+str(float(np.round(candidates_cross[i][0],2)))+","+str(float(round(float(candidates_cross[i][1]),2)))+","+str(np.round(upperlimit,1))+","+str(obs.exp)+","+str(obs.magsys)+","+str(obs.catalogcalib)+","+str(obs.filtercalib)+", #"+"RA: "+str(np.round(candidates_cross[i][2],5))+" dec: "+str(np.round(candidates_cross[i][3],5))+" ,STDPIPE mag "+str(np.round(candidates_cross[i][4],2))+","+str(np.round(candidates_cross[i][5],2))+" , "+str(author)+" , "+str(np.round(diff_time,3))+" delay, "+	str(humanize.precisedelta(diff_time*24.0*3600.0,minimum_unit="hours"))+"\n"
                print(str(float(round(float(candidates_cross[i][1]),2))))
                output.write(sentence)
        else:
             diff_time=(obs.mjd-GRB_time) #in seconds
             sentence=str(obs.obserdate)+","+str(obs.mjd)+","+str(obs.instru)+","+str(obs.filters)+","+str(0.0)+","+str(0.0)+","+str(np.round(upperlimit,1))+","+str(obs.exp)+","+str(obs.magsys)+","+str(obs.catalogcalib)+","+str(obs.filtercalib)+", #"+"RA: "+str(0.0)+" dec: "+str(0.0)+" ,STDPIPE mag "+str(0.0)+","+str(0.0)+" , "+str(author)+" , "+str(np.round(diff_time,3))+" delay, "+	str(humanize.precisedelta(diff_time*24.0*3600.0,minimum_unit="hours"))+"\n"
             print(sentence)
             output.write(sentence)			
...
def create_html(repo_photo):
    with open(os.path.join(repo_photo, "index_results.html"), "w") as fhtml:
        ...
        for detec in detecs:
            html += f"<h2>{detec}</h2>\n"
            pngfiles = glob.glob( os.path.join(repo_photo, detec, "*WITHINTARGET.png"))
...
def analyse_image(fits_file,target_ra,target_dec,target_radius,GRB_time,repo_res,images_folder,verbose,results_logs,force, config_data):
				...
				if show_upper=="True":
										print("########## UPPER ################")
										upperlimit,PLOT_UPPER=upper.calculate_upper(fits_file_comp,obs,config_data)
										plt.clf()
										plt.plot(PLOT_UPPER[0], PLOT_UPPER[1], '.', label=PLOT_UPPER[2])
										plt.plot(PLOT_UPPER[3],PLOT_UPPER[4],'r.', PLOT_UPPER[5], label=PLOT_UPPER[6])
										...
										#plt.show()
										#print(repo_photo)
										plt.savefig(os.path.join(repo_photo, obs.instru+"_"+obs.filters+"_"+str(np.round(obs.mjd,3))+"_upperlimit.png"))
										plt.show()
										plt.clf()
				...
				if force=="True":
					print("########## FORCE ################")
					candidates_cross=force_photometry(target_ra, target_dec, target_radius,fits_file_comp,obs,mask_simage_threshold,cat_ref,cat_col_mag_color,cat_col_mag1_color,cat_col_mag2_color,filter_ref)
			
				################ CROSS-MATCH ################################"
				else:
					if config_data["cross_match"] == "True":
						print( "########## CROSS ################ ="*20 + " Cross-Match " + "="*20)
						detection = "cat"
						repo_cat = os.path.join(repo_photo, "Cross-Match")
						res = detection_source(fits_file_comp,repo_cat,detection,obs,verbose,mask_simage_threshold,show_preprocimg,show_masksimg,cat_ref,stars_threshold,filter_ref,cat_col_mag_color,cat_col_mag1_color,cat_col_mag2_color,show_calib,show_candidate,template_type,template_ref,show_photorefimage,cat_colsous_mag,show_diffimg,config_data["sextractor"],target_ra,target_dec,target_radius,config_data)
				...
				if config_data["soustraction"] == "True":
							...
							res = detection_source(fits_file_comp,repo_sous,detection,obs,verbose,mask_simage_threshold,show_preprocimg,show_masksimg,cat_ref,stars_threshold,filter_ref,cat_col_mag_color,cat_col_mag1_color,cat_col_mag2_color,show_calib,show_candidate,template_type,template_ref,show_photorefimage,cat_colsous_mag,show_diffimg,config_data["sextractor"],target_ra,target_dec,target_radius,config_data)
				...
				create_logs(repo_photo,fits_file,obs,candidates_cross,upperlimit,config_data)
...
def force_photometry(target_ra, target_dec, target_radius,filename,obs,mask_simage_threshold,cat_ref,cat_col_mag_color,cat_col_mag1_color,cat_col_mag2_color,filter_ref):
	...
	image, header = cutouts.crop_image_centered(image_old, target_obj['x'], target_obj['y'], image_old.shape[0]/3.0, header=header_old)
	...
	mask |= cmask
	mask=mask*False
	obj = photometry.get_objects_sextractor(image, mask=mask, aper=5.0, gain=gain, edge=10)
	fwhm = np.median(obj['fwhm'][obj['flags'] == 0])
	obj = photometry.measure_objects(obj, image, mask=mask, fwhm=fwhm, gain=gain, aper=1.0, bkgann=[5, 7], sn=3, verbose=True)
	print(len(obj), 'objects properly measured')
	...
	cat = catalogs.get_cat_vizier(center_ra, center_dec, center_sr, cat_ref, filters={filter_ref:'<'+"21"})
	print(len(cat), 'catalogue stars')
	...
	if wcs is not None:
	       ...
	       header.update(wcs.to_header(relax=True))
	#stop
	m = pipeline.calibrate_photometry(obj, cat, sr=1/3600, cat_col_mag=cat_col_mag_color, cat_col_mag1=cat_col_mag1_color, cat_col_mag2=cat_col_mag2_color, max_intrinsic_rms=0.02, order=2, verbose=True)
	...
	# Forced photometry
	target_obj = photometry.measure_objects(target_obj, image, mask=mask, fwhm=fwhm, aper=1.0, bkgann=[5,7], sn=None, verbose=verbose, gain=gain)
	print(target_obj)
	target_obs=np.array(target_obj)

	# Assign calibrated magnitude
	target_obj['mag_calib'] = target_obj['mag'] + m['zero_fn'](target_obj['x'], target_obj['y'], target_obj['mag'])
	target_obj['mag_calib_err'] = np.hypot(target_obj['magerr'], m['zero_fn'](target_obj['x'], target_obj['y'], target_obj['mag'], get_err=True))

	# 5-sigma limit
	target_obj['mag_limit'] = -2.5*np.log10(5*target_obj['fluxerr']) + m['zero_fn'](target_obj['x'], target_obj['y'], target_obj['mag'])

	candidates_cross.append([target_obj['mag_calib'], target_obj['mag_calib_err'], target_obj['ra'], target_obj['dec'],target_obj['mag_calib'], target_obj['mag_calib_err']])
	#print(candidates_cross)
	#plt.title('GRB2210')
	#plots.imshow(images)
	#plt.show()
...
def process_image(fits_file, target_ra, target_dec, target_radius, event_time, res_folder, images_folder, verbose, res_logs, force, config):
    ...
    analyse_image(fits_file, target_ra, target_dec, target_radius, event_time, res_folder, images_folder, verbose, res_logs, force, config)
    try:
...
def main():
				...
				for fits_file in fits_list:
								if select_img in fits_file:
												if verbose:
													print_configuration(hostname, author, sextractor_default, res_folder, images_folder, fits_file, res_logs)
												process_image(fits_file, target_ra, target_dec, target_radius, event_time, res_folder, images_folder, verbose, res_logs, force,config_data)
